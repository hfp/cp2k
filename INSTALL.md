...
- Installing OpenCL and preparing the runtime environment
  ...
  - The environment variable `ACC_OPENCL_VERBOSE=2` prints information at runtime of CP2K about kernels
    generated and used (and thereby checks the installation).
- Building CP2K with OpenCL-based DBCSR
  ...
  - For manually writing an ARCH-file, add `-D__OPENCL` and `-D__DBCSR_ACC` to `CFLAGS` and add
    ...
    In fact, `GPUVER` limits tuned parameters to the specified GPU, and by default all tuned parameters
