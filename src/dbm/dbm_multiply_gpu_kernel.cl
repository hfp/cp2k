 ...
 */
kernel void process_batch_kernel(double alpha, int ntasks, int split,
                                 int task_offset,
  ...
  const int batchsize = (ntasks * split + work_size - 1) / work_size;
  const int i0 = idx * batchsize, i1 = i0 + batchsize;
  double vec[16];

  for (int i = i0; i < i1; ++i) {
    const int tid = i % ntasks, sid = i % split;
    ...
    if (sid < task.m) {
      ...
      for (int k = 0; k < task.k; ++k) {
        const double a = a_data[IDX(sid, k, task.offset_a, task.m, task.k)];
        for (int n = 0; n < task.n; ++n) {
      ...
      for (int n = 0; n < task.n; ++n) {
        ACCUMULATE(&c_data[IDX(sid, n, task.offset_c, task.m, task.n)],
                   alpha * vec[n]);
        vec[n] = ZERO; /* reset */
      }
#if 0
      printf("idx=%i tid=%i sid=%i\n", idx, tid, sid);
#endif
    }
  }
}
