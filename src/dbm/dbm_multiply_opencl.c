...
#error "OpenCL kernel code not found!"
  ...
  result |= c_dbcsr_acc_opencl_set_kernel_ptr(kernel, 6, batch.memory);
  result |= c_dbcsr_acc_opencl_set_kernel_ptr(kernel, 7, adata.memory);
  result |= c_dbcsr_acc_opencl_set_kernel_ptr(kernel, 8, bdata.memory);
  result |= c_dbcsr_acc_opencl_set_kernel_ptr(kernel, 9, cdata.memory);
